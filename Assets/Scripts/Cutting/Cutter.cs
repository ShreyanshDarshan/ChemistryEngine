...
public class Cutter : MonoBehaviour
{
    ...
    public static void Cut(GameObject originalGameObject, GameObject cutGameObject, Vector3 contactPoint, Vector3 cutNormal)
    {
        if (isBusy)
            return;
        ...
        Plane cutPlane = new Plane(Vector3.Scale(originalGameObject.transform.InverseTransformDirection(-cutNormal), originalGameObject.transform.localScale), originalGameObject.transform.InverseTransformPoint(contactPoint));
        ...
        GameObject right = cutGameObject;
        ...
        if (right.GetComponent<MeshRenderer>() == null)
        {
            right.AddComponent<MeshRenderer>();
        }

        Material[] mats = new Material[finishedRightMesh.subMeshCount];
        for (int i = 0; i < finishedRightMesh.subMeshCount; i++)
        {
            mats[i] = originalGameObject.GetComponent<MeshRenderer>().material;
        }
        right.GetComponent<MeshRenderer>().materials = mats;
        if (right.GetComponent<MeshFilter>() == null)
        {
